...
func (tx *Tx) init() {
	tx.AccessTokens = NewAccessTokensClient(tx.config)
	tx.AuthorizeCodes = NewAuthorizeCodesClient(tx.config)
	tx.BlacklistedJTIs = NewBlacklistedJTIsClient(tx.config)
	tx.Clients = NewClientsClient(tx.config)
	tx.IDSessions = NewIDSessionsClient(tx.config)
	tx.IssuerPublicKeys = NewIssuerPublicKeysClient(tx.config)
...
// applies a query, for example: AccessTokens.QueryXXX(), the query will be executed
